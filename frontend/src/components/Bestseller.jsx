...
function Bestseller() {
  ...
  // Extract unique categories and authors from the bestseller list
  const uniqueCategories = Array.from(new Set(list.filter(book => book.bestseller).map(book => book.category)));
  const uniqueAuthors = Array.from(new Set(list.filter(book => book.bestseller).map(book => book.author)));
  ...
  const filteredBooks = list.filter(book => {
    ...
    return book.bestSeller && categoryMatch && authorMatch;
  });
  ...
  return (
    <div className="max-w-screen-2xl container mx-auto md:px-20 px-4 mt-12">
      ...
      <p className="text-center mb-6 text-lg">
        Discover our best-selling books, handpicked for book lovers who appreciate top-notch storytelling and captivating narratives.
      </p>
      ...
      {/* Display Filtered Bestseller Books */}
